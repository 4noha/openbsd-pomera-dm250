...
経路 B / 自動化に失敗したときの代替。jcs 原典手順そのまま (`disklabel -E`
を明示的に呼ぶ)。一時期「`fdisk -g` 後に partition `a` が自動 expose される
ので disklabel 不要」 と書いていたが、実機 7.9 snapshot で auto-expose が
動かないケースが報告されたため撤回した — `disklabel -E` は
**unconditional に実行**するのが正解。
...
# OpenBSD slice 内に partition 'a' (4.2BSD) を切る (jcs 原典)
doas sh -c 'echo -e "a\n\n\n\n\nw\nx" | disklabel -E sd1'
