...
| `pomera-suspend.c` | `lid-watch` から 2 時間蓋閉じ後に呼ぶ deep suspend (`APM_IOC_SUSPEND` ioctl 直叩き、 armv7 base に `apm(8)` バイナリが無いので自作) |
| `pomera-lid-watch.sh` | 蓋閉じで backlight off → 2h 蓋閉じ継続で `pomera-suspend` で deep sleep → 蓋開で backlight 復元。 `sysctl machdep.lidaction=0` 前提 |
...
| `run-trial.sh` | `wifi-suspend-test.sh` を label 付き (例 `v3-pure-1`/`v3-diag-2`) で 1 trial 走らせるラッパー。 6-trial 対照実験 (`v3-pure` × 3 / `v3-diag` × 3) を ssh セッションから一括で叩く用 |
...
| `mlterm-fb-patches/` | mlterm 3.8.3 (`x11/mlterm`) の framebuffer backend に当てる 3 本の patch (emoji ゼロ除算修正、 32 bpp shadow buffer + 絵文字/アイコン描画修正、 DECSET 2026 同期出力)。 ビルドは `mlterm-fb-patches/README.md` |
