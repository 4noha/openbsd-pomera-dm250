...
# v3.6+ (2026-06-01) — Hack Nerd Font Mono の Font Awesome 4 glyph
# (BMP PUA U+E000-F8FF) に統一。 framebuffer mlterm-fb で確実に出る。
...
_bat() {
    ...
    # OCV correction → factory 21-point OCV table → SOC (5%-step linear interp)
    p=$(awk -v v="$v" -v i="$i" -v s="$s" -v R="$_BAT_R" '
        BEGIN {
            if      (s == "charging")    ocv = (v - i * R) * 1000   # → mV
